...
@EnableJpaRepositories(entityManagerFactoryRef = "provider1EntityManagerFactory", transactionManagerRef = "provider1TransactionManager", basePackages = {"io.opentelemetry.example.trade" })
public class Provider1DbConfig {
	...
	public LocalContainerEntityManagerFactoryBean entityManagerFactory(EntityManagerFactoryBuilder builder,
			@Qualifier("provider1DataSource") DataSource dataSource) {
		return builder.dataSource(dataSource).packages("io.opentelemetry.example.trade").persistenceUnit("provider1").build();
